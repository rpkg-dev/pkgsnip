...
#' ```
#'
#' @param id Snippet identifier. One of `r ls_file_snips()$id |> pal::as_md_vals() |> cli::ansi_collapse(last = " or ")`
#'
...
#' argument of [roxy_lbl()], [param_lbl()], [return_lbl()], etc.
#'
...
roxy_lbls <- function(type = "default") {
  ...
  data_roxy_lbls |>
    ...
    dplyr::group_by(id) |>
    dplyr::group_modify(\(d, k) if (nrow(d) == 1L) d else d |> dplyr::filter(type != "default")) |>
    dplyr::ungroup()
}
```
...
#' 
#' @param id Label identifier. See [roxy_lbls()] for possible values.
...
#' @param ... Further named arguments used to tailor the label to your needs. Not all labels require additional arguments, see [roxy_lbls()] for an overview.
